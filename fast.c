...
/********************* INIT **********************************/ 
int lm_init(blk_t SLC_blk_num,blk_t MLC_blk_num, blk_t extra_num)   
{
  int i;

  total_blk_num = SLC_blk_num + MLC_blk_num;
  BMT = (int *)malloc(sizeof(int) * total_blk_num);
  PMT = (struct LogMap*)malloc(sizeof(struct LogMap)*extra_num);
  total_log_blk_num = extra_num;

  if ((BMT== NULL) || (PMT == NULL)) { return -1; }

  memset(BMT, -1, sizeof(int) * total_blk_num);
